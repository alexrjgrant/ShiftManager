...
//        HTTP_GET("wsShifts.php?all=1").then(JSON.parse).then(displayAllShifts).catch(handleError); //Promise String
     HTTP_GET("wsShifts.php?all=1&SU=0&"+queryString).then(JSON.parse).then(displayAllShifts).catch(handleError); //Promise String   
    ...
    var queryString = new URLSearchParams(new FormData(document.getElementById("frm"))).toString();
     ...
     HTTP_GET("wsShifts.php?all=1&"+queryString).then(JSON.parse).then(displayAllShifts).catch(handleError); //Promise String   
...
{    
        HTTP_GET("wsShifts.php?all=0").then(JSON.parse).then(displayShifts).catch(handleError); //Promise String
    f=1;
}
function curShiftInit()
{  
       
    HTTP_GET("wsShifts.php?SU=1").then(JSON.parse).then(displayCurShifts).catch(handleError); //Promise String
}
function msgsInit()
{
     HTTP_GET("wsGetMsg.php").then(JSON.parse).then(displayMsgs).catch(handleError); //Promise String
}
        ...
        [varSID,varDate,varStart,varEnd,varS,varUID].forEach ( data => 
        {
            ...
            {            
                ...
                if (confirm(`Are you sure? Once accepted, the shift becomes your responcibillity to atend or find cover\n\n DATE : ${varDate} \n TIME : ${varStart} - ${varEnd} `)) 
                {
                    ...
                    HTTP_POST("wsClaimShift.php",curShift.ShiftID,"accept").then(JSON.parse).catch(handleError); 
    ...
    shifts.forEach( curShift => 
    {
        ...
        else if(varAv == 0)
        {
            ...
            {            
                ...
                {
                    removeItem("table");
                    HTTP_POST("wsClaimShift.php?",curShift.ShiftID,"swap").then(JSON.parse).catch(handleError);
